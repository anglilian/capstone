...
var mymap = L.map('map', {zoomControl: false}).setView([3.188, 101.69106], zoom);
...
var markerHarun = L.marker(mapMarkers[0].coordinates).addTo(mymap).bindPopup(mapMarkers[0].label).openPopup();

var markerVar = L.marker(mapMarkers[1].coordinates).addTo(mymap).bindPopup(mapMarkers[1].label);
...
var stepDivObserver = new IntersectionObserver(observerCallback, stepDivOptions);
    ...
    entries.forEach(entry=>{
        if (entry.isIntersecting){
            ...
            if (id == 1){
                ...
                var markerPoly = L.polygon([mapMarkers[id]["coordinatePoints"]]).addTo(mymap);
...
const mapDiv = document.querySelector("#scroll");
